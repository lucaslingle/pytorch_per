...
@tc.no_grad()
def _collect_footage(env, q_network, device, max_frames):
    frames = []
    s_t = env.reset()
    for t in range(0, max_frames):
        s_t = tc.tensor(s_t).float().unsqueeze(0).to(device)
        a_t = q_network.sample(
            x=s_t,
            epsilon=0.01)
        a_t = int(a_t.squeeze(0).detach().cpu().numpy())
        s_tp1, r_t, done_t, _ = env.step(action=a_t)
        if done_t:
            s_tp1 = env.reset()
...
def save_video(env, q_network, device, checkpoint_dir, run_name):
